...
class Agent(ABC):
    ...
    def __init__(
        ...
        # 新增：Shell 命令执行工具
        if self.config.bash_enabled and self.tool_registry:
            self._register_bash_tool()

        # 新增：文件搜索工具
        if self.config.glob_enabled and self.tool_registry:
            self._register_glob_tool()

        if self.config.grep_enabled and self.tool_registry:
            self._register_grep_tool()
...
（已压缩，保留最近 {self.config.min_retain_rounds} 轮完整对话）"""
    ...
    def _register_bash_tool(self):
        """注册 BashTool（Shell 命令执行工具）

        自动注册逻辑，在 __init__ 中调用（如果启用）
        """
        from ..tools.builtin.bash_tool import BashTool

        working_dir = str(self.working_dir) if hasattr(self, 'working_dir') else "."

        bash_tool = BashTool(
            working_dir=working_dir,
            default_timeout=self.config.bash_default_timeout,
            blocked_commands=self.config.bash_blocked_commands or None,
        )

        self.tool_registry.register_tool(bash_tool)

    def _register_glob_tool(self):
        """注册 GlobTool（文件模式匹配工具）
        ...
        glob_tool = GlobTool(project_root=project_root)

        self.tool_registry.register_tool(glob_tool)

    def _register_grep_tool(self):
        """注册 GrepTool（文件内容搜索工具）
        ...
        grep_tool = GrepTool(project_root=project_root)

        self.tool_registry.register_tool(grep_tool)
    ...
    def _create_light_llm(self) -> AgentsLLM:
        """创建轻量模型 LLM 实例
