...
class Config(BaseModel):
    ...
    # Shell 命令执行工具配置
    bash_enabled: bool = False  # 是否启用 Bash 工具（默认关闭，需显式开启）
    bash_default_timeout: int = 120  # Bash 命令默认超时（秒）
    bash_blocked_commands: list = []  # 额外的命令黑名单

    # 文件搜索工具配置
    glob_enabled: bool = True  # 是否启用 Glob 工具
    grep_enabled: bool = True  # 是否启用 Grep 工具

    # PlanAgent 计划检查点配置（中断恢复）
    plan_checkpoint_enabled: bool = True  # 是否启用计划检查点持久化
    plan_checkpoint_dir: str = "memory/checkpoints"  # 计划检查点保存目录
