...
class OpenAIAdapter(BaseLLMAdapter):
    ...
    def invoke(self, messages: List[Dict], **kwargs) -> LLMResponse:
        ...
        try:
            ...
            raise AgentsException(f"OpenAI API调用失败: {str(e)}")
    ...
    def stream_invoke(self, messages: List[Dict], **kwargs) -> Iterator[str]:
        ...
        try:
            ...
            raise AgentsException(f"OpenAI API流式调用失败: {str(e)}")
    ...
    async def astream_invoke(self, messages: List[Dict], **kwargs) -> AsyncIterator[str]:
        ...
        try:
            ...
            raise AgentsException(f"OpenAI API异步流式调用失败: {str(e)}")
    ...
    def invoke_with_tools(self, messages: List[Dict], tools: List[Dict],
        ...
        兼容两种返回模式：
        1. 标准 OpenAI 非流式响应（response.choices[0].message 可用）
        2. 某些网关在非流式请求下仍返回流式 SSE，此时降级为流式收集
        """
        if not self._client:
            self._client = self.create_client()
        ...
        try:
            ...
            # ---- 降级：网关返回了字符串或流式数据 ----
            # 某些兼容网关在 stream=False 时仍返回 SSE 流
            # 此时使用流式接口重新请求，并从流中重建 tool_calls
            return self._invoke_with_tools_stream_fallback(
                messages, tools, tool_choice, start_time, **kwargs
            )

        except AttributeError:
            # response 是字符串等非预期类型 -> 降级到流式重建
            return self._invoke_with_tools_stream_fallback(
                messages, tools, tool_choice, start_time, **kwargs
            )
            ...
            # 如果非流式失败，尝试流式降级
            if "stream" in str(e).lower() or "choices" in str(e).lower():
                return self._invoke_with_tools_stream_fallback(
                    messages, tools, tool_choice, start_time, **kwargs
                )
            raise AgentsException(f"OpenAI Function Calling调用失败: {str(e)}")

    def _invoke_with_tools_stream_fallback(
        self,
        messages: List[Dict],
        tools: List[Dict],
        ...
        **kwargs
        ...
        """流式降级：使用 stream=True 请求，从流中收集 tool_calls

        适用于某些 OpenAI 兼容网关在 stream=False 时仍返回流式数据的场景。
        从 SSE chunk 中逐步拼接 tool_calls 的 name 和 arguments。
        """
        try:
            response = self._client.chat.completions.create(
                model=self.model,
            ...
            content_parts = []
            # tool_calls 按索引收集：index -> {id, name, arguments}
            ...
            for chunk in response:
                ...
                if delta_tool_calls:
                    for dtc in delta_tool_calls:
                        idx = dtc.index if hasattr(dtc, 'index') and dtc.index is not None else 0
                        if idx not in tool_calls_map:
                            tool_calls_map[idx] = {"id": "", "name": "", "arguments": ""}

                        # tool call id（仅第一个 chunk 携带）
                        if hasattr(dtc, 'id') and dtc.id:
                            tool_calls_map[idx]["id"] = dtc.id

                        # function name（仅第一个 chunk 携带）
                        if hasattr(dtc, 'function') and dtc.function:
                            if hasattr(dtc.function, 'name') and dtc.function.name:
                                tool_calls_map[idx]["name"] += dtc.function.name
                            if hasattr(dtc.function, 'arguments') and dtc.function.arguments:
                                tool_calls_map[idx]["arguments"] += dtc.function.arguments
            ...
            for idx in sorted(tool_calls_map.keys()):
                ...
                if tc_data["name"]:  # 只有有名字的才是有效 tool call
                    tool_calls.append(ToolCall(
                        id=tc_data["id"] or f"call_{idx}_{int(time.time()*1000)}",
                        name=tc_data["name"],
            ...
            raise AgentsException(f"OpenAI Function Calling流式降级调用失败: {str(e)}")
...
class AnthropicAdapter(BaseLLMAdapter):
    ...
    def invoke(self, messages: List[Dict], **kwargs) -> LLMResponse:
        ...
        try:
            ...
            raise AgentsException(f"Anthropic API调用失败: {str(e)}")
    ...
    def stream_invoke(self, messages: List[Dict], **kwargs) -> Iterator[str]:
        ...
        try:
            ...
            raise AgentsException(f"Anthropic API流式调用失败: {str(e)}")
    ...
    def invoke_with_tools(self, messages: List[Dict], tools: List[Dict], **kwargs) -> LLMToolResponse:
        ...
        try:
            ...
            raise AgentsException(f"Anthropic工具调用失败: {str(e)}")
...
class GeminiAdapter(BaseLLMAdapter):
    ...
    def invoke(self, messages: List[Dict], **kwargs) -> LLMResponse:
        ...
        try:
            ...
            raise AgentsException(f"Gemini API调用失败: {str(e)}")
    ...
    def stream_invoke(self, messages: List[Dict], **kwargs) -> Iterator[str]:
        ...
        try:
            ...
            raise AgentsException(f"Gemini API流式调用失败: {str(e)}")
    ...
    def invoke_with_tools(self, messages: List[Dict], tools: List[Dict], **kwargs) -> LLMToolResponse:
        ...
        try:
            ...
            raise AgentsException(f"Gemini工具调用失败: {str(e)}")
